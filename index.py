...
try:
    from lightweight_ai_engine import (
        get_ai_response,
...
try:
    from experts.complete_16_experts_v2_enhanced_20250823 import (
        EnhancedComplete16ExpertAI,
    )
...
try:
    from experts.complete_16_experts_v3_multimodel_20250823 import (
        EnhancedComplete16ExpertAI_v3,
    )
...
try:
    from experts.complete_16_experts_v4_improved_quality_20250823_190858 import (
        MultimodalExpertAI,
    )
...
try:
    from experts.complete_16_experts_v5_global_expansion_20250823 import (
        GlobalExpertSystemV5,
    )
...
try:
    from experts.complete_16_experts_v6_ultimate_global_20250823 import (
        ComprehensiveExpertSystemV6,
    )
...
if VERCEL_ENV:
    print(
        "🚀 Vercel 모드: v7.0 실시간 멀티모달 시스템 비활성화 (음성 라이브러리 호환성)"
    )
    EXPERTS_V7_AVAILABLE = False
else:
    try:
        from experts.complete_16_experts_v7_real_time_multimodal_20250823 import (
            GlobalExpertSystemV7,
        )
...
if VERCEL_ENV:
    ...
    try:
        from experts.complete_16_experts_v8_cosmic_multimodal_20250823 import (
            UniversalAISystemV8,
        )
...
try:
    from experts.complete_16_experts_v9_dna_personalized_20250823 import (
...
class UltraLightAIManager:
    ...
    def __init__(self):
        ...
        if EXPERTS_V3_AVAILABLE:
            try:
                self.expert_ai_v3 = EnhancedComplete16ExpertAI_v3(
                    db_path="expert_ai_v3.db"
                )
    ...
    def get_casual_response(self, query):
        ...
        if "잘 지내" in query_lower:
            return (
                "네, 잘 지내고 있어요! 감사합니다. 😊 어떤 이야기를 나누고 싶으신가요?"
            )
    ...
    def get_expert_response(
        self, query, expert_name="assistant", mode="deep", user_id="default_user"
    ):
        """🚀 2단계: Enhanced 16명 전문가 시스템 응답 생성 (개인화 + 성능 모니터링)"""

        print(
            f"🔧 모드 설정: {mode} ({'심화탐구' if mode == 'deep' else '창의협업' if mode == 'creative' else '기본'})"
        )
        ...
        if hasattr(self, "use_16_experts_v2") and self.use_16_experts_v2:
            try:
                ...
                if (
                    expert_response
                    and expert_response
                    != "죄송합니다. 해당 전문 분야를 찾을 수 없습니다."
                ):
                    print(
                        f"✅ Enhanced 전문가 응답 성공! (길이: {len(expert_response)}자)"
                    )
        ...
        if hasattr(self, "use_16_experts") and self.use_16_experts:
            try:
                ...
                expert_response = self.expert_ai.generate_expert_response(
                    query, expert_name
                )
                if (
                    expert_response
        ...
        if "이전 질문" in query and "후속 질문:" in query:
            ...
            if len(parts) == 2:
                ...
                return self._generate_contextual_response(
                    current_question, expert_name, previous_context
                )
        ...
        try:
            ...
            return self._generate_advanced_response(
                query, expert_name, search_info, mode
            )
        except Exception as e:
...
**'{question}'**에 대해 {previous_context} 주제를 더 깊이 있게 알아볼게!
...
{self._generate_additional_insights(question, expert_name)}
    ...
    def _generate_advanced_response(
        self, query, expert_name, search_info="", mode="deep"
    ):
        ...
        try:
            ...
            if self.use_lightweight_ai and LIGHTWEIGHT_AI_AVAILABLE:
                ...
                expert_type = (
                    "technical"
                    if "기술" in expert_name
                    or expert_name in ["assistant", "tech", "data_analyst"]
                    else (
                        "business"
                        if expert_name in ["marketing", "sales", "business", "startup"]
                        else "general"
                    )
                )

                ai_response = get_ai_response(
                    user_input=query, expert_type=expert_type, expert_name=expert_name
                )
                ...
                if ai_response and len(ai_response) > 20:
                    print(
                        f"✅ 경량 AI 엔진 응답 성공: {len(ai_response)}자 ({mode} 모드)"
                    )
                    return ai_response
    ...
    def _generate_dynamic_response(
        ...
        # 전문가별 관점 적용
        expert_perspective = self._get_expert_perspective(
            expert_name, question_analysis
        )
        ...
        if search_info:
            ...
            response_parts.append(
                f"{search_info[:300]}..." if len(search_info) > 300 else search_info
            )
            response_parts.append("")
        ...
        if question_analysis["type"] == "how_to":
            response_parts.append(
                self._generate_how_to_response(query, expert_name, question_analysis)
            )
        elif question_analysis["type"] == "what_is":
            response_parts.append(
                self._generate_explanation_response(
                    query, expert_name, question_analysis
                )
            )
        elif question_analysis["type"] == "comparison":
            response_parts.append(
                self._generate_comparison_response(
                    query, expert_name, question_analysis
                )
            )
        elif question_analysis["type"] == "advice":
            response_parts.append(
                self._generate_advice_response(query, expert_name, question_analysis)
            )
        else:
            response_parts.append(
                self._generate_general_response(query, expert_name, question_analysis)
            )

        # 전문가별 특화 인사이트 추가 (모드별 차별화)
        response_parts.append(
            self._generate_expert_insights(query, expert_name, question_analysis)
        )

        # 실행 방안 (질문에 따라 동적 생성, 모드별 차별화)
        response_parts.append(
            self._generate_dynamic_action_plan(query, expert_name, question_analysis)
        )
        ...
        print(
            f"🔄 동적 응답 생성 완료: {len(final_response)}자 (질문타입: {question_analysis['type']}{search_tag})"
        )
    ...
    def _analyze_question(self, query):
        ...
        # 복잡성 레벨
        complexity = "basic"
        if len(keywords) > 10 or any(
            word in query for word in ["전략", "분석", "구체적", "상세"]
        ):
            complexity = "advanced"
        elif len(keywords) > 5:
...
{expert_name} 관점에서 체계적으로 비교분석해볼게!
...
'{query}'에 대해 {analysis['domain']} 분야 전문가로서 분석해볼게!
...
정기적 리뷰를 통해 계획을 업데이트하고 최적화해나가겠습니다.
    ...
    def _generate_basic_response(self, query, expert_name, mode="deep"):
        ...
        if mode == "creative":
            creative_responses = {
                "AI전문가": f"🎨 AI도깨비야! '{query}'에 대해 창의적으로 접근해보자면, AI랑 인간이 협업해서 만들 수 있는 완전 새로운 가능성들을 탐구해볼 수 있을 것 같아! 같이 브레인스토밍하면서 혁신적인 아이디어 발굴해보는 거 어때?",
                "마케팅왕": f"🎨 마케팅도깨비야! '{query}'를 창의적으로 분석해보니까, 스토리텔링이랑 감성 마케팅으로 고객들과 진짜 연결고리를 만들어보는 게 어떨까? 같이 독창적인 캠페인 아이디어 구상해보자!",
                "의료AI전문가": f"🎨 의료AI도깨비야! '{query}'에 대해 창의적으로 생각해보니까, 환자 중심의 혁신적인 솔루션을 같이 고민해볼 수 있을 것 같아! 의료진이랑 환자가 모두 만족할 수 있는 새로운 접근법을 탐구해보자!",
                "재테크박사": f"🎨 투자도깨비야! '{query}'를 창의적으로 접근해보니까, 전통적인 투자 방식을 넘어서는 새로운 기회들을 같이 발굴해볼 수 있을 것 같아! 혁신적인 투자 전략을 협업으로 만들어보는 거 어떨까?",
                "창업컨설턴트": f"🎨 창업도깨비야! '{query}'에 대해 창의적으로 생각해보니까, 기존 비즈니스 모델의 틀을 깨는 혁신적인 아이디어를 같이 브레인스토밍해볼 수 있을 것 같아! 파괴적 혁신의 가능성을 탐구해보자!",
                "개발자멘토": f"🎨 개발도깨비야! '{query}'를 창의적으로 접근해보니까, 기술적 한계를 뛰어넘는 새로운 솔루션을 같이 구상해볼 수 있을 것 같아! 혁신적인 개발 방법론이랑 아이디어를 협업으로 만들어보자!",
            }
            return creative_responses.get(
                expert_name,
                f"🎨 도깨비 전문가야! '{query}'에 대해 창의적이고 협업적인 관점에서 같이 탐구해보자!",
            )
        else:  # mode == "deep"
            deep_responses = {
                "AI전문가": f"🔍 AI도깨비야! '{query}'에 대해 심층 분석해보니까, 현재 AI 기술의 근본적 메커니즘부터 실제 구현 세부사항까지 꼼꼼하게 파헤쳐볼 수 있을 것 같아! 기술적 깊이랑 실무적 통찰 모두 알려줄게!",
                "마케팅왕": f"🔍 마케팅도깨비야! '{query}'를 심화 분석해보니까, 시장 동향 분석, 소비자 행동 심리학, 데이터 기반 성과 측정까지 다 포함해서 전략적 인사이트 제공해줄 수 있어!",
                "의료AI전문가": f"🔍 의료AI도깨비야! '{query}'에 대해 깊이 있는 분석해보니까, 의학적 근거, 임상 데이터, 기술적 구현 방안까지 전문적 관점에서 체계적으로 설명해줄 수 있어!",
                "재테크박사": f"🔍 투자도깨비야! '{query}'를 심층 분석해보니까, 재무 이론, 시장 구조 분석, 리스크 관리 방법론까지 포함한 전문적 투자 전략을 상세히 알려줄 수 있어!",
                "창업컨설턴트": f"🔍 창업도깨비야! '{query}'에 대해 심화 컨설팅해보니까, 사업 모델 설계, 시장 진입 전략, 성장 단계별 핵심 과제까지 체계적으로 분석해줄 수 있어!",
                "개발자멘토": f"🔍 개발도깨비야! '{query}'에 대해 심층 기술 분석해보니까, 아키텍처 설계, 성능 최적화, 보안 고려사항까지 포함한 전문적 개발 가이드 제공해줄 수 있어!",
            }
            return deep_responses.get(
                expert_name,
                f"🔍 도깨비 전문가 관점에서 '{query}'에 대한 심층적이고 체계적인 분석을 해줄게!",
            )
...
def select_expert_by_query(query):
    ...
    if any(
        ...
        for keyword in ["블록체인", "blockchain", "암호화폐", "비트코인", "crypto"]
    ):
        selected_expert = "블록체인도깨비"
        expert_matched = True
    elif any(
        keyword in query_lower
        for keyword in ["마케팅", "marketing", "광고", "브랜딩", "홍보"]
    ):
        selected_expert = "마케팅왕"
        expert_matched = True
    elif any(
        keyword in query_lower
        for keyword in ["의료", "건강", "병원", "의사", "치료", "진단"]
    ):
        selected_expert = "의료AI전문가"
        expert_matched = True
    elif any(
        keyword in query_lower
        for keyword in ["투자", "재테크", "주식", "펀드", "금융", "돈"]
    ):
        selected_expert = "재테크박사"
        expert_matched = True
    elif any(
        keyword in query_lower
        for keyword in ["창업", "스타트업", "사업", "비즈니스", "기업"]
    ):
        selected_expert = "창업컨설턴트"
        expert_matched = True
    elif any(
        keyword in query_lower
        for keyword in ["개발", "프로그래밍", "코딩", "개발자", "프로그램"]
    ):
        selected_expert = "개발자멘토"
        expert_matched = True
    elif any(
        keyword in query_lower
        for keyword in ["ai", "인공지능", "머신러닝", "딥러닝", "알고리즘"]
    ):
    ...
    if not expert_matched:
        ...
        if any(
            ...
            keyword in combined_text
            for keyword in ["창업", "스타트업", "사업", "비즈니스", "기업", "회사"]
        ):
            return "창업컨설턴트"
        elif any(
            keyword in combined_text
            ...
            keyword in combined_text
            for keyword in ["ai", "인공지능", "머신러닝", "딥러닝", "알고리즘", "기술"]
        ):
            return "AI전문가"
        else:
...
def manage_conversation_context(conversation_id, message, expert_name, response):
    ...
    if len(conversation_context[conversation_id]["messages"]) > 10:
        conversation_context[conversation_id]["messages"] = conversation_context[
            conversation_id
        ]["messages"][-10:]
...
def get_context_aware_expert_selection(message, conversation_id, goblin_id=1):
    ...
    print(
        f"🔍 컨텍스트 분석 시작: '{message}' (대화ID: {conversation_id}, 도깨비: {goblin_id})"
    )
    ...
    if goblin_expert != question_expert:
        print(
            f"� 도깨비{goblin_id} 전문가: {goblin_expert} vs 질문 분석: {question_expert}"
        )
...
def chat():
    ...
    try:
        ...
        print(
            f"🚀 Enhanced 전문가 채팅 요청: {expert} - {query[:50]}... (모드: {mode}, 사용자: {user_id})"
        )
        ...
        # 사용된 전문가 시스템 확인
        if (
            hasattr(real_ai_manager, "use_16_experts_v2")
            and real_ai_manager.use_16_experts_v2
        ):
            expert_system_info = "Enhanced 16명 전문가 시스템 v2.0"
            system_version = "V2-ENHANCED"
        elif (
            hasattr(real_ai_manager, "use_16_experts")
            and real_ai_manager.use_16_experts
        ):
...
def chat_advanced():
    ...
    try:
        ...
        if conversation_id in conversation_context:
            print(
                f"📝 기존 컨텍스트 발견: {len(conversation_context[conversation_id]['messages'])}개 메시지"
            )
            print(
                f"📝 현재 전문가: {conversation_context[conversation_id]['current_expert']}"
            )
            print(
                f"📝 현재 주제: {conversation_context[conversation_id]['current_topic']}"
            )
        else:
            print(f"📝 새 컨텍스트 생성")
        ...
        expert_name, previous_topic = get_context_aware_expert_selection(
            message, conversation_id, goblin_id
        )
        ...
        if expert_name == "일반대화":
            ...
            if previous_topic:
                ...
                response = real_ai_manager._generate_contextual_response(
                    message, expert_name, previous_topic
                )
                ...
                response = real_ai_manager.get_expert_response(
                    message, expert_name, mode
                )
            ...
            # 🧬 DNA 개인화 적용
            final_response = dna_system.apply_dna_personalization(
                response_with_empathy, user_id
            )

        # 대화 컨텍스트 저장
        manage_conversation_context(
            conversation_id, message, expert_name, final_response
        )
...
def performance_analytics():
    ...
    try:
        ...
        print(f"❌ 성능 분석 오류: {e}")
        return (
            jsonify({"error": "성능 분석 오류가 발생했습니다.", "success": False}),
            500,
        )
    ...
    try:
        ...
        experts = [
            ...
            },
            {
                ...
                "name": "AI도깨비",
                "emoji": "🧠",
                "description": "AI 응용 및 구현 전문",
                "specialty": "AI 응용 기술",
                "personality": "창의적이고 혁신적인 사고",
            ...
            },
            {
                ...
                "name": "블록체인도깨비",
                "emoji": "⛓️",
                "description": "블록체인 및 암호화폐 전문",
                "specialty": "블록체인 & 암호화폐",
                "personality": "신중하고 보안 중심",
            ...
            },
            {
                ...
                "name": "데이터과학박사도깨비",
                "emoji": "📊",
                "description": "빅데이터 분석 및 ML 전문",
                "specialty": "데이터 사이언스",
                "personality": "체계적이고 분석적",
            ...
            },
            {
                ...
                "name": "게임개발도깨비",
                "emoji": "🎮",
                "description": "게임 기획 및 개발 전문",
                "specialty": "게임 개발",
                "personality": "재미있고 창의적",
            ...
            },
            {
                ...
                "name": "양자컴퓨팅도깨비",
                "emoji": "⚛️",
                "description": "양자컴퓨팅 연구 전문",
                "specialty": "양자 컴퓨팅",
                "personality": "미래지향적이고 과학적",
            ...
            },
            {
                ...
                "name": "로봇공학도깨비",
                "emoji": "🤖",
                "description": "로봇 설계 및 제어 전문",
                "specialty": "로봇 공학",
                "personality": "정밀하고 실용적",
                "avatar": "/static/avatar_ai_philosopher_happy_203148.png",
                "free": True,
                "price": 0,
                "category": "ai_tech",
            },
            {
                ...
                "name": "사이버보안도깨비",
                "emoji": "🛡️",
                "description": "정보보안 및 해킹 방어 전문",
                "specialty": "사이버 보안",
                "personality": "신중하고 경계심 강함",
                "avatar": "/static/avatar_ai_philosopher_happy_203548.png",
                "free": True,
                "price": 0,
                "category": "ai_tech",
            },
            {
                ...
                "name": "우주항공도깨비",
                "emoji": "🚀",
                "description": "항공우주 기술 전문",
                "specialty": "우주 항공",
                "personality": "도전적이고 모험적",
                "avatar": "/static/avatar_ai_philosopher_happy_203813.png",
                "free": True,
                "price": 0,
                "category": "ai_tech",
            },
            ...
            },
            {
                ...
                "name": "경영학박사도깨비",
                "emoji": "💼",
                "description": "기업 경영 전략 전문",
                "specialty": "경영 전략",
                "personality": "체계적이고 리더십 있음",
                "avatar": "/static/avatar_ai_philosopher_happy_204241.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "컨설팅박사도깨비",
                "emoji": "🎯",
                "description": "경영 컨설팅 전문",
                "specialty": "경영 컨설팅",
                "personality": "분석적이고 해결 지향적",
                "avatar": "/static/avatar_ai_philosopher_neutral_202955.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "경제학박사도깨비",
                "emoji": "📊",
                "description": "거시/미시 경제 분석 전문",
                "specialty": "경제 분석",
                "personality": "논리적이고 예측적",
                "avatar": "/static/avatar_ai_philosopher_curious_194719.png",
                "free": True,
                "price": 0,
                "category": "business",
            ...
            },
            {
                ...
                "name": "국제무역도깨비",
                "emoji": "🌍",
                "description": "글로벌 무역 및 수출입 전문",
                "specialty": "국제 무역",
                "personality": "글로벌하고 개방적",
                "avatar": "/static/avatar_ai_philosopher_neutral_204651.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "인사관리박사도깨비",
                "emoji": "👥",
                "description": "인사 관리 및 조직 개발 전문",
                "specialty": "인사 관리",
                "personality": "소통 지향적이고 배려심 있음",
                "avatar": "/static/avatar_ai_philosopher_happy_203148.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "정책개발도깨비",
                "emoji": "🏛️",
                "description": "정책 기획 및 공공 정책 전문",
                "specialty": "정책 개발",
                "personality": "공익 지향적이고 체계적",
                "avatar": "/static/avatar_ai_philosopher_happy_203548.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "영업학박사도깨비",
                "emoji": "💪",
                "description": "영업 전략 및 고객 관리 전문",
                "specialty": "영업 전략",
                "personality": "적극적이고 설득력 있음",
                "avatar": "/static/avatar_ai_philosopher_happy_203813.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "쇼핑박사도깨비",
                "emoji": "🛍️",
                "description": "소비자 트렌드 및 쇼핑 전문",
                "specialty": "소비자 트렌드",
                "personality": "트렌드에 민감하고 실용적",
                "avatar": "/static/avatar_ai_philosopher_happy_203828.png",
                "free": True,
                "price": 0,
                "category": "business",
            },
            {
                ...
                "name": "창업학박사도깨비",
                "emoji": "🚀",
                "description": "창업 전략 및 스타트업 전문",
                "specialty": "창업 전략",
                "personality": "도전적이고 혁신적",
                "avatar": "/static/avatar_ai_philosopher_curious_194719.png",
                "free": True,
                "price": 0,
                "category": "business",
            ...
            },
            {
                ...
                "name": "여행컨설팅도깨비",
                "emoji": "✈️",
                "description": "여행 기획 및 관광 전문",
                "specialty": "여행 컨설팅",
                "personality": "모험적이고 서비스 정신 있음",
                "avatar": "/static/avatar_ai_philosopher_neutral_202955.png",
                "free": True,
                "price": 0,
                "category": "business",
            ...
            },
            {
                ...
                "name": "창의기획도깨비",
                "emoji": "💡",
                "description": "크리에이티브 디렉션 전문",
                "specialty": "창의 기획",
                "personality": "혁신적이고 상상력 풍부",
                "avatar": "/static/avatar_ai_philosopher_curious_194719.png",
                "free": True,
                "price": 0,
                "category": "creative",
            },
            {
                ...
                "name": "문화기획도깨비",
                "emoji": "🎭",
                "description": "문화 콘텐츠 기획 전문",
                "specialty": "문화 기획",
                "personality": "문화적 감수성이 높음",
                "avatar": "/static/avatar_ai_philosopher_neutral_202955.png",
                "free": True,
                "price": 0,
                "category": "creative",
            },
            {
                ...
                "name": "패션스타일링도깨비",
                "emoji": "👗",
                "description": "패션 트렌드 및 스타일링 전문",
                "specialty": "패션 스타일링",
                "personality": "세련되고 트렌디",
                "avatar": "/static/avatar_ai_philosopher_happy_203148.png",
                "free": True,
                "price": 0,
                "category": "creative",
            },
            {
                ...
                "name": "음악제작도깨비",
                "emoji": "🎵",
                "description": "음악 제작 및 사운드 디자인 전문",
                "specialty": "음악 제작",
                "personality": "감성적이고 예술적",
                "avatar": "/static/avatar_ai_philosopher_happy_203548.png",
                "free": True,
                "price": 0,
                "category": "creative",
            },
            {
                ...
                "name": "스토리텔링도깨비",
                "emoji": "📖",
                "description": "스토리 창작 및 콘텐츠 기획 전문",
                "specialty": "스토리텔링",
                "personality": "상상력 풍부하고 따뜻함",
                "avatar": "/static/avatar_ai_philosopher_happy_203813.png",
                "free": True,
                "price": 0,
                "category": "creative",
            },
            {
                ...
                "name": "문학박사도깨비",
                "emoji": "✍️",
                "description": "문학 창작 및 글쓰기 전문",
                "specialty": "문학 창작",
                "personality": "깊이 있고 성찰적",
                "avatar": "/static/avatar_ai_philosopher_happy_203828.png",
                "free": True,
                "price": 0,
                "category": "creative",
            ...
            # 🏥 Healthcare (5명)
            {
                ...
                "name": "바이오도깨비",
                "emoji": "🧬",
                "description": "생명공학 및 바이오 기술 전문",
                "specialty": "생명공학",
                "personality": "과학적이고 정밀함",
                "avatar": "/static/avatar_ai_philosopher_neutral_204651.png",
                "free": True,
                "price": 0,
                "category": "healthcare",
            },
            {
                ...
                "name": "건강관리도깨비",
                "emoji": "💪",
                "description": "건강 관리 및 피트니스 전문",
                "specialty": "건강 관리",
                "personality": "활동적이고 에너지 넘침",
                "avatar": "/static/avatar_ai_philosopher_happy_203148.png",
                "free": True,
                "price": 0,
                "category": "healthcare",
            },
            {
                ...
                "name": "의료AI전문가",
                "emoji": "⚕️",
                "description": "의료 AI 및 디지털 헬스케어 전문",
            ...
            },
            {
                ...
                "name": "신약개발도깨비",
                "emoji": "💊",
                "description": "신약 개발 및 제약 연구 전문",
                "specialty": "신약 개발",
                "personality": "연구 중심적이고 인내심 있음",
                "avatar": "/static/avatar_ai_philosopher_happy_203548.png",
                "free": True,
                "price": 0,
                "category": "healthcare",
            },
            {
                ...
                "name": "웰니스박사도깨비",
                "emoji": "🧘",
                "description": "웰니스 및 정신 건강 전문",
                "specialty": "웰니스",
                "personality": "평온하고 치유적",
                "avatar": "/static/avatar_ai_philosopher_happy_203813.png",
                "free": True,
                "price": 0,
                "category": "healthcare",
            ...
            },
            {
                ...
                "name": "교육도깨비",
                "emoji": "📚",
                "description": "교육 방법론 및 커리큘럼 전문",
                "specialty": "교육 방법론",
                "personality": "체계적이고 인내심 있음",
                "avatar": "/static/avatar_ai_philosopher_curious_194719.png",
                "free": True,
                "price": 0,
                "category": "education",
            },
            {
                ...
                "name": "언어교육도깨비",
                "emoji": "🗣️",
                "description": "언어 학습 및 교육 전문",
                "specialty": "언어 교육",
                "personality": "소통 지향적이고 친근함",
                "avatar": "/static/avatar_ai_philosopher_neutral_202955.png",
                "free": True,
                "price": 0,
                "category": "education",
            ...
            # 🌱 Lifestyle (2명)
            {
                ...
                "name": "사회혁신도깨비",
                "emoji": "🌍",
                "description": "사회 문제 해결 및 혁신 전문",
                "specialty": "사회 혁신",
                "personality": "이상주의적이고 진보적",
                "avatar": "/static/avatar_ai_philosopher_neutral_204651.png",
                "free": True,
                "price": 0,
                "category": "lifestyle",
...
def upload_and_analyze():
    ...
    try:
        ...
        if "file" not in request.files:
            return (
                jsonify({"status": "error", "error": "파일이 선택되지 않았습니다."}),
                400,
            )
        ...
        if file.filename == "":
            return jsonify({"status": "error", "error": "파일명이 없습니다."}), 400
...
def get_supported_formats():
    ...
    if not DOCUMENT_ANALYSIS_AVAILABLE:
        return (
            jsonify(
                {"status": "error", "error": "문서 분석 시스템이 사용할 수 없습니다."}
            ),
            503,
        )
...
def analyze_text_content():
    ...
    try:
        ...
        if not data or "text" not in data:
            return (
                jsonify(
                    {"status": "error", "error": "분석할 텍스트가 제공되지 않았습니다."}
                ),
                400,
            )
        ...
        if len(text.strip()) < 10:
            return (
                jsonify({"status": "error", "error": "분석할 텍스트가 너무 짧습니다."}),
                400,
            )
    ...
    def _generate_recommendations(self, fto, comt, actn3):
        ...
        return {
            "nutrition": {
                ...
                "supplements": (
                    ["B-complex", "마그네슘"] if fto == "AA" else ["카르니틴", "크롬"]
                ),
            },
            "exercise": {
            ...
            "cognitive": {
                ...
                "optimal_environment": (
                    "조용한 환경" if comt == "Met/Met" else "활발한 환경"
                ),
            },
        }
