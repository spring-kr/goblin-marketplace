...
class ResponseContextManager:
    ...
    def create_expertise_based_response(
        ...
        response_parts = []
        greeting = self._generate_greeting(agent_info.get("name", "도깨비"), context.current_time)
        response_parts.append(greeting)

        expertise = agent_info.get("expertise") or [agent_info.get("field", "전문")]
        analysis = self._analyze_with_expertise(question, expertise, context.depth_level)
        response_parts.append(analysis)

        for section in sections:
            response_parts.append(self._generate_section_content(section, question, agent_info, context))

        response_parts.append(self._generate_closing(context_patterns["follow_up"], agent_info.get("field", "전문")))
    ...
    def _analyze_with_expertise(self, question: str, expertise_areas: List[str], depth_level: int) -> str:
        relevant = expertise_areas[:1]
        return f"귀하의 질문에 대해 {', '.join(relevant)}의 관점에서 {'심층적인' if depth_level>1 else '기본적인'} 분석을 진행했습니다."

    def _generate_section_content(self, section: str, question: str, agent_info: dict, context: ContextInfo) -> str:
    ...
    def _generate_closing(self, is_follow_up: bool, field: str) -> str:
        return (f"추가적인 {field} 관련 질문이 있으시다면 언제든 물어보세요. 🌟" if is_follow_up 
