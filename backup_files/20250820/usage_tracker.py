...
class UsageTracker:
    ...
    def get_statistics(self, days: int = 7) -> Dict[str, Any]:
        try:
            ...
            recent_logs = [log for log in logs if datetime.datetime.fromisoformat(log["timestamp"]) >= cutoff_date]
            if not recent_logs:
            ...
            agent_names = {"math": "🧮 수학 도깨비", "physics": "⚛️ 물리학 도깨비", "chemistry": "🧪 화학 도깨비", "biology": "🧬 생물학 도깨비", "engineering": "⚙️ 공학 도깨비", "assistant": "🤖 업무 도우미 도깨비", "marketing": "📈 마케팅 도깨비", "startup": "🚀 스타트업 도깨비"}
            return {
                "period": f"최근 {days}일",
                ...
                "average_question_length": round(sum(log["question_length"] for log in recent_logs) / total_usage, 1),
                "peak_hour": (max(hourly_usage, key=hourly_usage.get) if hourly_usage else "없음"),
                "most_popular_agent": (agent_names.get(max(agent_usage, key=agent_usage.get), "없음") if agent_usage else "없음"),
            }
        except Exception as e:
