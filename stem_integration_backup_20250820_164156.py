...
class STEMIntegration:
    ...
    def _get_fortune_response(self, question: str, agent_profile: dict) -> str:
        ...
        if any(keyword in question_lower for keyword in love_keywords):
            ...
            # 맥락에 맞는 인사말 생성
            greeting = "반갑습니다" if analysis['time_context']['mood'] == "활기찬" else "안녕하세요"
            
            # 에너지 흐름에 따른 메시지 생성
            energy_message = {
                "상승세": "좋은 기운이 상승하고 있어요",
                "안정세": "차분하고 안정적인 기운이 감지됩니다",
                "균형잡힌": "균형 잡힌 에너지가 돋보이는 시기예요",
                "변화의": "새로운 변화의 기운이 느껴져요",
                "기회의": "특별한 기회가 다가오고 있어요"
            }[analysis['energy_flow']]
            
            return f"""🔮 {greeting}! 운세 도깨비입니다.
...
⏰ **현재 운세의 흐름:**
{analysis['time_context']['energy']} 시작된 하루,
{"주말의 여유로운 분위기를 즐기시면 좋겠어요." if analysis['is_weekend'] else "평일의 생동감 있는 에너지가 당신의 매력을 더욱 빛나게 할 거예요."}\n"""
