...
"""
                ...
                # 실제 검색 기반 전문 분석 - 질문 유형별 맞춤 분석
                query_lower = query.lower()

                # 비즈니스 전략 관련 질문
                if any(
                    keyword in query_lower
                    for keyword in ["전략", "strategy", "비즈니스", "사업", "경영"]
                ):
                    return f"""
🎓 **비즈니스 전략 전문 분석**
...
### 1. **전략적 사고 프레임워크**
**Porter의 5 Forces 분석**
- **공급업체 협상력:** 원재료/서비스 공급업체의 영향력
- **구매자 협상력:** 고객의 가격 협상 및 대안 선택권
- **신규 진입자 위협:** 새로운 경쟁자의 시장 진입 가능성
- **대체재 위협:** 기존 제품/서비스를 대체할 수 있는 솔루션
- **기존 경쟁자 간 경쟁:** 현재 시장 내 경쟁 강도

### 2. **SWOT 전략 매트릭스**
```
내부 요인:
- Strengths (강점): 핵심 역량, 브랜드 파워, 기술력
- Weaknesses (약점): 리소스 부족, 시장 인지도, 운영 효율성

외부 요인:
- Opportunities (기회): 시장 성장, 규제 완화, 기술 발전
- Threats (위협): 경쟁 심화, 경기 침체, 규제 강화
```

### 3. **Blue Ocean 전략**
**가치 혁신을 통한 무경쟁 시장 창출**
- **제거 (Eliminate):** 업계가 당연시하는 요소 중 제거할 것
- **감소 (Reduce):** 업계 표준보다 대폭 감소시킬 요소
- **증가 (Raise):** 업계 표준보다 대폭 높일 요소  
- **창조 (Create):** 업계에서 제공하지 않는 새로운 요소

### 4. **실행 로드맵**
**단계별 전략 실행 계획**
- **1분기:** 시장 분석 완료, 핵심 지표 설정
- **2분기:** 파일럿 프로그램 실행, 초기 성과 측정
- **3분기:** 전면 실행, 실시간 모니터링 체계 구축
- **4분기:** 성과 평가, 다음 해 전략 수립

### 5. **성공 지표 (KPI)**
- **재무 지표:** 매출 성장률 20%+, 영업이익률 15%+
- **고객 지표:** NPS 50+, 고객 재구매율 60%+  
- **운영 지표:** 시장 점유율 증가, 프로세스 효율성 개선

*🎓 Village Chief v3.0 - 비즈니스 전략 박사급 전문 분석 시스템*
"""

                # 경제/금융 관련 질문
                elif any(
                    keyword in query_lower
                    for keyword in ["경제", "금융", "투자", "트렌드", "시장"]
                ):
                    return f"""
🌐 **경제 트렌드 & 시장 분석**
...
### 1. **거시경제 전망**
**글로벌 경제 환경**
- **성장률 전망:** 주요국 GDP 성장률 2.5-3.2% 예상
- **인플레이션:** 목표 수준(2%) 근접, 안정화 단계 진입
- **금리 정책:** 주요국 중앙은행 완화적 스탠스 유지
- **환율 동향:** 달러 강세 완화, 신흥국 통화 회복세

### 2. **산업별 성장 동력**
**Technology & Innovation**
- **AI/ML:** 생성형 AI 상용화 가속, 시장 규모 1,800억 달러
- **반도체:** 메모리 수요 회복, 시스템 반도체 성장
- **클라우드:** 하이브리드 클라우드 확산, 엣지 컴퓨팅 부상

**Green Economy**
- **재생에너지:** 태양광/풍력 설치 용량 25% 증가
- **전기차:** 글로벌 EV 판매 3,000만 대 돌파 예상
- **ESG 투자:** 지속가능 금융 자산 50조 달러 돌파

### 3. **투자 기회 분석**
**성장 섹터 포트폴리오**
```
고성장 기대 섹터:
1. AI/로봇공학 (연평균 성장률 35%+)
2. 바이오헬스케어 (연평균 성장률 22%+)  
3. 클린테크 (연평균 성장률 28%+)
4. 디지털 전환 (연평균 성장률 18%+)
```
...
### 4. **리스크 요인**
**주요 위험 요소**
- **지정학적 긴장:** 공급망 재편, 무역 분쟁 재점화 가능성
- **기후변화:** 극한 기후로 인한 경제적 충격
- **부채 증가:** 정부/기업 부채 부담 확대
- **기술 격차:** 디지털 전환 속도 차이로 인한 양극화
...
*🌐 Village Chief v3.0 - 경제/금융 박사급 전문 분석 시스템*
"""

                # 기술/혁신 관련 질문
                elif any(
                    keyword in query_lower
                    for keyword in ["기술", "혁신", "innovation", "tech", "개발"]
                ):
                    return f"""
🚀 **기술 혁신 동향 분석**
...
**생성형 AI 진화**
- **GPT-5/Claude-4:** 멀티모달 AI 상용화 
- **AI 에이전트:** 자율적 업무 처리 시스템 구현
- **Edge AI:** 디바이스 내장형 AI 칩셋 보급 확산
- **AI 민주화:** No-Code/Low-Code AI 플랫폼 대중화

**실무 적용 분야**
```
산업별 AI 도입률:
- 금융업: 78% (리스크 관리, 개인화 서비스)
- 제조업: 65% (예측 정비, 품질 관리)  
- 헬스케어: 58% (진단 보조, 신약 개발)
- 교육: 45% (개인화 학습, 콘텐츠 생성)
```
...
**양자컴퓨팅 실용화**
- **IBM/Google:** 1,000큐비트 시스템 상용화
- **암호화 혁신:** 양자 내성 암호 알고리즘 개발
- **최적화 문제:** 물류/금융 최적화 솔루션 적용
- **화학 시뮬레이션:** 신소재/신약 개발 가속화

### 3. **Biotechnology & Longevity**
**바이오 기술 혁신**
- **CRISPR 3.0:** 정밀 유전자 편집 기술 고도화
- **세포 치료:** CAR-T, 줄기세포 치료제 확산
- **디지털 헬스:** 웨어러블 진단, 원격 모니터링
- **수명 연장:** 노화 억제 치료제 임상 성과

### 4. **Sustainable Technology**
**지속가능 기술**
- **탄소 포집:** DAC(Direct Air Capture) 기술 상용화
- **대체 단백질:** 배양육, 식물성 단백질 시장 확장
- **순환경제:** 재활용/업사이클링 기술 혁신
- **청정에너지:** 페로브스카이트 태양전지, 고체 배터리
...
**성공 지표**
- **특허 출원:** 핵심 기술 IP 확보 수준
- **상용화 속도:** R&D에서 제품 출시까지 기간
- **시장 침투율:** 타겟 시장 내 점유율 달성도
- **파트너십:** 글로벌 기업과의 협력 관계 구축

*🚀 Village Chief v3.0 - 기술혁신 박사급 전문 분석 시스템*
"""
...
- **영업이익률:** 영업이익 / 매출 × 100 (업계 평균: 8-15%)
- **순이익률:** 순이익 / 매출 × 100 (목표: 10%+)
- **ROA:** 순이익 / 총자산 × 100 (우수: 15%+)
- **ROE:** 순이익 / 자기자본 × 100 (우수: 20%+)
...
- **EV/EBITDA:** 기업가치 / 이자·세금·감가상각비 차감 전 이익
- **DCF (현금흐름할인법):** 미래 현금흐름의 현재가치

### 3. **재무 계획 & 예측**
**재무 모델링 핵심 요소**
```
3년 재무 계획:
Year 1: 매출 50억 → 영업이익률 12%
Year 2: 매출 75억 → 영업이익률 15%  
Year 3: 매출 100억 → 영업이익률 18%
...
### 4. **리스크 관리**
**재무 리스크 요소**
- **유동성 리스크:** 현금흐름 부족, 단기 자금 조달 어려움
- **신용 리스크:** 매출채권 회수 불능, 거래처 부도
- **시장 리스크:** 환율 변동, 금리 변동, 원자재 가격 변동
- **운영 리스크:** 핵심 인력 이탈, 시스템 장애

**리스크 대응 전략**
```
리스크 완화 방안:
1. 현금 보유: 월 운영비의 6-12개월분
2. 신용 관리: 매출채권 한도 설정, 보험 가입
3. 헤징: 환율/금리 변동 대응 파생상품 활용
4. 보험: 핵심 리스크에 대한 보험 가입
```

### 5. **성과 개선 전략**
**수익성 제고 방안**
- **매출 증대:** 신제품 출시, 신시장 진출, 가격 최적화
- **비용 절감:** 프로세스 개선, 자동화, 아웃소싱
- **자산 효율성:** 재고 최적화, 매출채권 관리 강화
- **자본 구조:** 최적 부채비율 유지, 자본비용 최소화

*📊 Village Chief v3.0 - 재무성과 박사급 전문 분석 시스템*
"""

                # 일반적인 비즈니스 질문에 대한 포괄적 분석
                else:
                    return f"""
🎯 **비즈니스 전문 분석**
...
### 1. **핵심 개념 정의**
"{query}"는 현대 비즈니스 환경에서 성공을 위한 핵심 요소입니다.

### 2. **실무 적용 방법론**
**Framework 기반 접근법**
- **분석 단계:** 현황 파악 → 문제 정의 → 대안 도출
- **계획 단계:** 목표 설정 → 전략 수립 → 실행 계획
- **실행 단계:** 리소스 배분 → 진행 관리 → 성과 측정
- **개선 단계:** 결과 분석 → 피드백 수집 → 지속 개선
...
2. 실행 역량 (Execution Excellence)  
3. 혁신 능력 (Innovation Capability)
4. 리더십 (Leadership & Culture)
```

### 4. **벤치마킹 & 모범 사례**
**Industry Best Practices**
- **글로벌 리더:** 선도 기업의 성공 전략 분석
- **동종업계:** 경쟁사 대비 차별화 포인트 발굴
- **트렌드 분석:** 최신 산업 동향과 미래 전망
- **혁신 사례:** 파괴적 혁신을 통한 시장 리더십 확보

### 5. **실행 로드맵**
**단계별 실행 계획**
```
Phase 1 (1-3개월): 현황 분석 및 전략 수립
- 시장 조사 및 경쟁 분석
- 내부 역량 평가
- 목표 설정 및 KPI 정의

Phase 2 (4-9개월): 핵심 이니셔티브 실행
- 우선순위 프로젝트 착수
- 팀 구성 및 리소스 배분
- 진행 상황 모니터링

Phase 3 (10-12개월): 성과 최적화
- 결과 분석 및 ROI 측정
- 성공 요인 공유 및 확산
- 지속 개선 체계 구축
```
...
### 6. **측정 지표**
**성과 측정 KPI**
- **정량 지표:** 매출, 수익성, 시장점유율, 고객만족도
- **정성 지표:** 브랜드 인지도, 직원 만족도, 혁신 역량
- **선행 지표:** 파이프라인, 신규 고객 획득, R&D 투자
- **후행 지표:** 실제 성과, 고객 유지율, 수익 실현

*🎯 Village Chief v3.0 - 비즈니스 전문 분석 시스템*
"""
...
def serve_index():
    ...
    return send_from_directory(web_dir, "simple-chat.html")
...
class VillageChiefLoader:
    def __init__(self):
        ...
        print("🎯 VillageChiefLoader 초기화 완료!")
...
def get_village_chief():
    ...
    if _village_chief_instance is None:
        _village_chief_instance = VillageChiefLoader()
    ...
    return jsonify(
        {
            ...
            "message": f"{len(function_list)}개 카테고리 로드 완료",
        }
    )
