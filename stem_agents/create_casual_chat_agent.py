...
"""
    ...
    # 일상대화 특화 응답 함수
    response_function = '''#!/usr/bin/env python3
"""
💬 일상대화 친화적 AI 에이전트 응답 생성 함수
...
if __name__ == "__main__":
    ...
    for i, message in enumerate(test_messages, 1):
        print(f"\\n🧪 테스트 {i}: {message}")
        response = generate_casual_conversation_response(message)
        print(f"🤖 응답: {response}")
        print("-" * 50)
'''
...
def run_web_interface():
    """웹 인터페이스 실행"""
    html_file = Path(__file__).parent / "{agent_id}_interface.html"
    if html_file.exists():
        print(f"🌐 {agent_name} 웹 인터페이스 실행 중...")
        print(f"📁 파일 위치: {{html_file}}")
        webbrowser.open(f"file://{{html_file.absolute()}}")
        print("✅ 브라우저에서 열렸습니다!")
    else:
        print("❌ HTML 인터페이스 파일을 찾을 수 없습니다.")

def test_response_function():
    """응답 함수 테스트"""
    try:
        from {agent_id}_response import generate_casual_conversation_response
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            files = [
                "config.json",
                "{agent_id}_system_prompt.txt", 
                "{agent_id}_response.py",
                "{agent_id}_interface.html",
...
'''
    ...
    with open(output_dir / "run_agent.py", "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    print(f"3. 통합 실행: run_agent.py 실행")
