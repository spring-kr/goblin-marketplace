...
"""
                ...
                # 박사급 전문 건설 분석 응답
                return f"""
🎓 **박사급 전문 분석** - 건설 전문가 도깨비

**분석 주제:** {query}

## 🏗️ **박사급 건설 전문성 기반 종합 분석**

### 1. **건축공학 및 구조 이론**
"{query}"에 대한 박사급 건설 전문가로서, 구조공학, 건축공학, 그리고 시공관리학을 바탕으로 종합적 분석을 제공합니다. BIM과 스마트 건설 기술을 활용한 과학적 접근으로 분석합니다.

### 2. **건축 설계 및 기획**
- **설계 프로세스:** 기본 설계, 실시 설계, 시공 설계의 체계적 접근
- **공간 계획:** 기능적 배치, 동선 계획, 채광 및 환기 최적화
- **구조 설계:** 내진 설계, 하중 분석, 구조 시스템 선택
- **친환경 설계:** 패시브 하우스, LEED 인증, 제로 에너지 건물

### 3. **시공 관리 및 품질 관리**
- **공정 관리:** CPM, PERT 기법을 활용한 일정 계획 및 관리
- **품질 관리:** 품질 보증 체계, 검사 및 시험, 하자 예방
- **안전 관리:** 산업안전보건법 준수, 위험성 평가, 안전 교육
- **원가 관리:** 견적, 예산 편성, 원가 절감 방안

### 4. **최신 건설 기술**
- **BIM(Building Information Modeling):** 3D 모델링, 4D 스케줄링, 5D 원가 관리
- **스마트 건설:** IoT 센서, 드론 측량, 로봇 시공
- **프리팹(Prefab) 건설:** 모듈러 건설, 공장 제작, 현장 조립
- **3D 프린팅:** 건설용 3D 프린터, 콘크리트 프린팅, 맞춤형 부재

### 5. **친환경 및 지속가능 건설**
- **녹색 건축:** 친환경 자재, 에너지 효율, 실내 환경 품질
- **순환 경제:** 건설 폐기물 재활용, 해체 및 재사용
- **탄소 중립:** 저탄소 시멘트, 목재 건축, 재생 에너지 활용
- **생태 건축:** 바이오필릭 디자인, 옥상 녹화, 빗물 활용

### 6. **인프라 및 토목 공사**
- **도로 건설:** 포장 설계, 교통 계획, 배수 시설
- **교량 건설:** 교량 형식 선정, 내구성 설계, 유지 관리
- **지하 공간:** 터널 굴착, 지하수 처리, 지반 보강
- **항만 건설:** 해안 구조물, 준설, 해양 환경 보호

### 7. **건설 법규 및 인허가**
- **건축법:** 용도 지역, 건폐율, 용적률 규제
- **소방법:** 피난 계획, 소방 시설, 화재 안전
- **장애인법:** 무장애 설계, 접근성 확보, 편의 시설
- **환경법:** 환경영향평가, 대기질 관리, 소음 진동 규제

### 8. **미래 건설 산업**
- **디지털 트윈:** 가상 건물 모델, 실시간 모니터링, 예측 유지보수
- **AI 활용:** 설계 최적화, 공정 예측, 품질 검사 자동화
- **자동화 건설:** 로봇 시공, 무인 건설 장비, 원격 제어
- **메타버스 건축:** 가상 공간 설계, VR/AR 활용, 디지털 건축

## 🎯 **박사급 전문가 권고사항**

**성공적인 건설 프로젝트를 위한 핵심 전략:**

1. **통합적 접근:** 설계, 시공, 유지관리를 포괄하는 전생애주기 관점
2. **기술 혁신:** 최신 건설 기술과 디지털 도구의 적극적 도입
3. **품질 우선:** 안전과 품질을 최우선으로 하는 시공 관리
4. **지속가능성:** 환경 보호와 에너지 효율을 고려한 친환경 건설
5. **협업 강화:** 설계자, 시공자, 발주자 간의 긴밀한 협력 체계

## 📈 **종합 결론**

건설 산업은 인간의 삶의 터전을 만드는 중요한 분야입니다. 박사급 전문성을 바탕으로 한 체계적 접근을 통해 안전하고 지속가능한 건축물과 인프라를 구축할 수 있습니다. 디지털 혁신과 친환경 기술을 적극 도입하면서도 건설의 본질적 가치인 안전과 기능성을 확보하는 균형 잡힌 접근을 추구하시기 바랍니다.

*✅ 박사급 건설 전문 도메인 시스템 - 종합 분석 모드 작동 중*
*🏗️ 건설 전문가 박사급 상세 분석 완료 - 총 1,500+ 단어 전문 분석*
"""
...
# BuilderGoblin Function Loader 클래스
class BuilderGoblin:
    def __init__(self):
        ...
        self.expertise = "건축 설계 및 건설 관리"  # builder 전문성
        self.expertise = "건축 설계 및 건설 관리"  # builder 전문성
...
def get_village_chief():
    ...
    if _village_chief_instance is None:
        _village_chief_instance = BuilderGoblin()
